...
class NaukriLoginClient:
    ...
    def _get_cookie(self, name: str) -> str | None:
        """Retrieve a cookie value by name from httpcloak's cookie list."""
        for cookie in self.session.get_cookies():
            if cookie.name == name:
                return cookie.value
        return None

    def _cookies_as_dict(self) -> dict:
        """Convert httpcloak's cookie list to a {name: value} dict."""
        return {c.name: c.value for c in self.session.get_cookies()}
    ...
    def login(self):
        ...
        self.naukri_session = NaukriSession(token, self._cookies_as_dict())
    ...
    def update_profile(self, headline: str = None, name: str = None, summary: str = None, key_skills: list = None):
        pid = self.fetch_profile_id()
        ...
        if key_skills is not None:
            profile_fields["keySkills"] = {
                "preferredKeySkills": [{"name": s} for s in key_skills]
            }
        ...
        logger.debug("update_profile response: %s", res.text[:500])
