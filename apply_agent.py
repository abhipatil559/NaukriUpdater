...
#
# Configuration:
#   Set USERNAME, PASSWORD, and GROQ_API_KEY in a .env file.
...
from src.client.profile_extractor import extract_profile, print_profile_summary
...
def build_queries(profile: dict) -> list:
    """
    Generate search queries from the extracted resume profile.

    Combines target_role_keywords with preferred locations.
    Falls back to [""] (all India) if no locations are specified.
    """
    keywords  = profile.get("target_role_keywords", profile.get("target_roles", []))
    locations = profile.get("preferred_locations", [""])

    if not keywords:
        keywords = ["software developer"]  # safe fallback
    if not locations:
        locations = [""]
    ...
    for kw in keywords:
        for loc in locations:
            queries.append({"keyword": kw, "location": loc})
    return queries


def fetch_all_jobs(jc: NaukriJobClient, profile: dict) -> list:

    BQUERIES = build_queries(profile)

    exp_years = int(profile.get("experience_years", 2))
    EXPERIENCE_LEVELS = [exp_years]
    PAGES   = 1
    JOB_AGE = 2
...
if __name__ == "__main__":
    ...
    # Step 0: extract candidate profile from resume.
    print_section_title("extracting candidate profile from resume")
    profile = extract_profile(resume_path, ai_key)
    print_profile_summary(profile)
    # Print raw JSON for full visibility
    display = {k: v for k, v in profile.items() if not k.startswith("_")}
    print(f"  {Fore.WHITE}{Style.DIM}{json.dumps(display, indent=2)}{Style.RESET_ALL}")
    ...
    # Step 2: fetch raw jobs from search API (queries from profile).
    jc   = NaukriJobClient(client)
    jobs = fetch_all_jobs(jc, profile)
    ...
    pipeline   = JobFilterPipeline2(groq_api_key=ai_key, profile=profile)
